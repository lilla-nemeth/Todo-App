...
function isPwLongEnough(req, res, next) {
  ...
  if (pw.length >= 6) {
    ...
    res
      .status(403)
      .json({ msg: 'The length of password should be at least 6 characters.' });
  }
}
...
app.post('/todos', authMw, (request, response) => {
  ...
  pool
    .query('INSERT INTO todos(title, userId) VALUES ($1, $2) RETURNING *', [
      title,
      userId,
    ])
    .then((res) => response.status(200).json(res.rows))
...
app.put('/todos/:id', authMw, (request, response) => {
  ...
  pool
    .query(
      'UPDATE todos SET title=$1, completed=$2, importance=$3 WHERE id=$4',
      [title, completed, importance, id]
    )
    .then((res) =>
...
app.post('/signup', [isEmail, isPwLongEnough], (request, response) => {
  ...
  pool
    .query(
      'INSERT INTO users (email, username, pw) VALUES ($1, $2, $3) RETURNING *',
      [email, username, encryptedPw]
    )
    .then((res) => {
